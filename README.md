...
<p align="center">
  ...
  <p align="center">
    A one-click and containerized setup for XUI.ONE, by <a href="https://shahidmalla.com">Shahid Malla</a>.
    <br />
    <br />
...
This repository provides a fully automated setup for **XUI.ONE**, a comprehensive IPTV panel used for capturing, delivering, and transcoding video content (enterprise video delivery, streaming, VOD, etc.). It includes:
...
> This repository is **private**. Only people you explicitly invite as collaborators can view or clone it.
...
1. **Automated Setup** — Installs and configures `XUI.ONE 1.5.12` on `Ubuntu 20.04 / 22.04 / 24.04`.
2. **Dependency Management** — Installs everything XUI.ONE needs (MySQL/MariaDB, Python, unzip, etc).
...
The installer downloads the XUI.ONE package from this repository's **GitHub Release** (not the old, dead third-party host), so the download link stays reliable and under your control.
...
```bash
wget https://raw.githubusercontent.com/shahidmallaofficial/xui-installer/main/install.sh -O install.sh && chmod +x install.sh && ./install.sh
```
...
1. **Clone the repository** (requires access, since this repo is private):

   ```bash
   git clone https://github.com/shahidmallaofficial/xui-installer.git
   cd xui-installer
   ```
...
3. **Access XUI.ONE:** open the URL/port shown by `docker compose logs -f xui` in your browser.
...
- Default web ports and service management are handled by XUI.ONE itself once installed.
...
- **Download fails:** confirm you (or your invited collaborators) have access to this private repo and that the [Release](../../releases) named in `install.sh` still exists.
